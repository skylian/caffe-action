...
void BatchNormLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // extract param
  BNParameter bn_param = this->layer_param().bn_param();
  var_eps_ = bn_param.var_eps();
  decay_ = bn_param.decay();
  moving_average_ = bn_param.moving_average();
  ...
  if (this->blobs_.size() > 0) {
    ...
    if (bn_param.has_scale_filler()) {
      scale_filler.reset(GetFiller<Dtype>(bn_param.scale_filler()));
    } else {
      FillerParameter filler_param;
      filler_param.set_type("constant");
      filler_param.set_value(1);
      LOG(INFO) << "scale filler not provided: use scale = 1.";
    ...
    if (bn_param.has_shift_filler()) {
      shift_filler.reset(GetFiller<Dtype>(bn_param.shift_filler()));
    } else {
      FillerParameter filler_param;
      filler_param.set_type("constant");
      filler_param.set_value(0);
      LOG(INFO) << "shift filler not provided: use shift = 0.";
      shift_filler.reset(GetFiller<Dtype>(filler_param));
    }
...
void BatchNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  switch (this->layer_param_.bn_param().bn_forward_mode()) {
    case BNParameter_BNForwardMode_NoMean:
      // ---------- scale ---------- //
      ...
      caffe_mul(buffer_blob_.count(), const_bottom_data,
      ...
      caffe_add(buffer_blob_.count(), const_top_data,
                buffer_blob_.cpu_data(), top_data);
      break;
    case BNParameter_BNForwardMode_CenterMean:
      // ---------- mean subtraction ---------- //
      ...
      if (this->phase_ == TRAIN) {
        caffe_cpu_axpby(batch_statistic_.count(), decay_,
                        batch_statistic_.cpu_data(), Dtype(1) - decay_,
      ...
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_ * channels_,
                            ...
                            Dtype(0), buffer_blob_.mutable_cpu_data());
      // substract mean
      caffe_add(buffer_blob_.count(), const_bottom_data,
                buffer_blob_.cpu_data(), top_data);

      // ---------- variance normalization ---------- //
      // put the squares of X - mean into buffer_blob_
      caffe_powx(buffer_blob_.count(), const_top_data, Dtype(2),
                 buffer_blob_.mutable_cpu_data());
      // statistic across spatial
      caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, height_ * width_,
                            Dtype(1. / (height_ * width_)),
      ...
      // add eps
      caffe_add_scalar(batch_statistic_.count(), var_eps_,
                       batch_statistic_.mutable_cpu_data());
      // std
      ...
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                            ...
                            spatial_sum_multiplier_.cpu_data(),
                            Dtype(0), buffer_blob_.mutable_cpu_data());
      caffe_mul(buffer_blob_.count(), const_top_data,
                buffer_blob_.cpu_data(), top_data);
      ...
      break;
    default:
      LOG(FATAL) << "Unknown BN forwardmode.";
  }
}
...
void BatchNormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  caffe_mul(buffer_blob_.count(), x_norm_.cpu_data(),
            const_top_diff, buffer_blob_.mutable_cpu_data());
  // statistic across spatial
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, height_ * width_,
  ...
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                        ...
                        spatial_sum_multiplier_.cpu_data(),
                        Dtype(0), buffer_blob_.mutable_cpu_data());
  caffe_mul(buffer_blob_.count(), const_top_diff,
  ...
  caffe_cpu_axpby(buffer_blob_.count(),
                  Dtype(1), buffer_blob_.cpu_data(),
                  Dtype(-1. / (num_ * height_ * width_)),
                  bottom_diff);

  // variance normalization
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                        num_, channels_, 1, Dtype(1),
                        batch_sum_multiplier_.cpu_data(), x_std_.cpu_data(),
                        Dtype(0), spatial_statistic_.mutable_cpu_data());
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                        num_ * channels_, height_ * width_, 1,
                        Dtype(1), spatial_statistic_.cpu_data(),
  ...
  caffe_div(buffer_blob_.count(), const_bottom_diff,
            buffer_blob_.cpu_data(), bottom_diff);
}
