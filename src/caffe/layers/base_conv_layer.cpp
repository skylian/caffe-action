...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	if (dynamic_conv) {
		...
		// input blobs starts from index 0
		id = 0;
	}
	CHECK_EQ(4, bottom[id]->num_axes()) << "Input must have 4 axes, "
...
void BaseConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
	...
	CHECK_EQ(4, bottom[id]->num_axes()) << "Input must have 4 axes, "
			<< "corresponding to (num, channels, height, width)";
	num_ = bottom[id]->num();
	height_ = bottom[id]->height();
	width_ = bottom[id]->width();
	CHECK_EQ(bottom[id]->channels(), channels_) << "Input size incompatible with"
			" convolution kernel.";
	// TODO: generalize to handle inputs of different shapes.
	for (int bottom_id = id+1; bottom_id < bottom.size(); ++bottom_id) {
