...
void SpatialProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_; ++n) {
      const Dtype* scale_data  = bottom[1]->cpu_data() + bottom[1]->offset(n);
      for (int c = 0; c < channels_; ++c) {
        Dtype* top_data          = top[0]->mutable_cpu_data() + top[0]->offset(n, c);
        const Dtype* bottom_data = bottom[0]->mutable_cpu_data() + bottom[0]->offset(n, c);
...
void SpatialProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[1]) {
    for (int n = 0; n < num_; ++n) {
      const Dtype* top_diff    = top[0]->cpu_diff() + top[0]->offset(n);
      const Dtype* bottom_data = bottom[0]->cpu_data() + bottom[0]->offset(n);
      Dtype* scale_diff        = bottom[1]->mutable_cpu_diff() + bottom[1]->offset(n);
