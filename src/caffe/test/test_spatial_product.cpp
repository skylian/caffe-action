...
TYPED_TEST(SpatialProductLayerTest,
  ...
  for (int n = 0; n < num; ++n) {
    const Dtype* scale = this->blob_bottom_scale_->cpu_data() + this->blob_bottom_scale_->offset(n);
    for (int c = 0; c < channel; ++c) {
      const Dtype* data = this->blob_bottom_data_->cpu_data() + this->blob_bottom_data_->offset(n, c);
      const Dtype* top_data = this->blob_top_->cpu_data() + this->blob_top_->offset(n, c);
